...
def t1(L):
    ...
    Hint: vstack 垂直堆放/hstack 水平堆放/dstack 深度堆放，沿第三个轴, no for loop
    """
...
def t2(X):
    ...
    Hints:
    1) np.linalg.eig 返回一个tuple(eigenvalues,eigenvectors) 每个eigenvector是存储为一列
    2) np.argmin
    3) Watch rows and columns!
    """
    Y=np.linalg.eig(X)
    min_index=np.argmin(Y[0])
    return Y[1][:,min_index]
...
def t9(q, k, v):
    ...
    """
    dist=np.sum((q-k)**2,axis=1,keepdims=True)
    weights=np.exp(-dist)
...
def t10(Xs):
    ...
    for i in range(len(Xs)):
        for j in range(len(Xs)):
            R[i,j]=np.linalg.norm(centroids[i]-centroids[j])
    return R
...
def t11(X):
    ...
    # D=norms_sq+norms_sq.T-2*dot_product

    return np.sqrt(np.maximum(0,np.sum(X**2,axis=1,keepdims=True)+np.sum(X**2,axis=1,keepdims=True).T-2*X@X.T))
...
def t12(X, Y):
    ...
    """
    # dot_product=X@Y.T
    # norm_sqX=np.sum(X**2,axis=1,keepdims=True)
    # norm_sqY=np.sum(Y**2,axis=1,keepdims=True)
    # D=norm_sqX+norm_sqY.T-2*dot_product
    return  np.sqrt(np.maximum(0,np.sum(X**2,axis=1,keepdims=True)+np.sum(Y**2,axis=1,keepdims=True).T-2*X@Y.T))
...
def t20(N, v):
    ...
    """
    jj,ii=np.meshgrid(np.arange(N),np.arange(N))
    numerator=np.abs(v[0]*jj+v[1]*ii+v[2])
    denominator=np.sqrt(v[0]**2+v[1]**2)
